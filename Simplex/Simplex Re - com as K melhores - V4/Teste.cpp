...
solCR* removePrimeiroLista(solCR* lst){
	lst = lst->prox;
	return lst;
}
solCR* reordenaLista(solCR* lst, int tam, float cbar[], float msimplex[]){
	solCR* novo = criaLista();
	
	while(lst != NULL){
		lst->cr = cbar[lst->indice] - lst->valor - msimplex[tam + lst->indice];
		novo = insereListaOrdenado(novo, lst->valor, lst->sol, lst->cr, lst->indice, tam);
		lst = lst->prox;
	}
	...
	Inicio:
			...
			while(!pare)
			{
				...
				else
				{
					...
					fprintf(teste, "MULTIPLICADOR SIMPLEX\n");
					for (int tst = 0; tst < totalItensDiscretizados; tst++)
						fprintf(teste, "%f\n", multiplicadorSimplex[tst]);
				...
				if (acabouKmelhores){
					...
					lista = reordenaLista(lista, totalItensDiscretizados, cost_bar, multiplicadorSimplex);
					...
					/*while(l2 != NULL){
						...
						for(int re = 0; re < totalItensDiscretizados; re++)
							if(l2->sol[re] > 0)
								printf("%d(%f)\t", dados[re].l, l2->sol[re]);
						printf("\n");
						l2 = l2->prox;
					}
					system("pause");*/
					...
					if(!acabouKmelhores){
						if ((lista != NULL) && (lista->cr < 0) && (lista->cr - g < 0.000001))
						{
							...
							indice = lista->indice;
							alfa = lista->sol;
							lista = removePrimeiroLista(lista);
						}
						else{
