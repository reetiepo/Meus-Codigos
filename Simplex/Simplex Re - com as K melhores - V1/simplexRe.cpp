...
#define dirArquivoEntrada "Arquivo/Entrada/Class10/D10d"
// Diretório dos arquivos de saida e relatórios
#define dirArquivoSaidaRelatorio "Arquivo/Saida/Class10/S-D10d"
...
void Troca(int tamanhoItem[], float qtdItem[], float custoItem[], float valorItem[], int i, int j)
{
	int auxInt;
	float auxFlot;
...
void Ordena(int tamanhoItem[], float qtdItem[], float custoItem[], float valorItem[], int qtdTiposItem)
{
	int i, j;
...
void Reordena(int tamanhoItem[], float qtdItem[], float custoItem[], float solucao[], int original[], int qtdTiposItem)
{
	int i, j;
...
float CalulaSolucaoInicial(float x[], int tamanhoItem[], float qtdItem[], int totalItens, int tamanhoBarra, int qtdTiposItem, bool *temRetalho)
{
	int j, i;
	float soma = 0;
	bool ok = true;
...
double CalculaMochila(int tamanhoBarra, int qtdTiposItem, int tamanhoItem[], float custoItem[], float qtdItem[], int totalItens, float solucao[])
{
	int i,j;
	int indice = 0;
	float *x = new float[totalItens];
	float *valor = new float[qtdTiposItem];
	int *troca = new int[qtdTiposItem];
	double total = 0;
	double sobra = 0;
	double G = 0;
	double auxG = 0;
...
Passo2:
	...
	temRetalho = false;
	//SOLUÇÃO INICIAL
	CalulaSolucaoInicial(x,tamanhoItem,qtdItem,totalItens,tamanhoBarra,qtdTiposItem,&temRetalho);
	//FIM SOLUÇÃO INICIAL
...
int main(int argc, char **argv)
{
	...
	//variáveis usadas nos relatórios
	double qtdd_obj = 0;
	double qtdd_padrao = 0;
	double tempo_total = 0;
	double perda_total = 0;
	double sobra_total = 0;
	double obj_perfeito = 0;
	double obj_com_perda = 0;
	double obj_com_sobra = 0;
	double obj_retalho = 0;
	double obj_padrao0 = 0;
	double obj_padrao1 = 0;
	long double tam_total_padrao = 0;
	long double tam_total = 0;

	int qtdd_bar_usadas;
	int qtdTiposItem, sujeira;
	int qtdRetalhos = 0;
	...
	for(numArquivoEntrada = comecarem; numArquivoEntrada <= qtdArquivosEntrada; numArquivoEntrada++)
	{
		...
		{			
			...
			for(i = 0; i < maxQtdTipoRetalho; i++)
			{
				switch (i)
				{
					case 0:	size_item[qtdTiposItem + i] = 500;
							break;
					case 1: size_item[qtdTiposItem + i] = 400;
							break;
					case 2: size_item[qtdTiposItem + i] = 300;
							break;				
				}
				...
				else
				{
					...
					for(i = 0; i < qtdTiposBarras; i++)
						if(amount_bar[i] > 0)
						{
							...
							z = CalculaMochila(size_bar[i],qtdTiposItem,size_item,valor,amount_item,totalItensDiscretizados,solucao);
			...
			while (j < tam_sol)
			{
				if(VerificaPadraoNulo(matsol,n,qtdTiposItem))
				{
					...
					if(vetsol[n] > 0.5)
					{
						...
						fprintf(out,"(%d)(%d)\t%4.2f\t",n,size_bar[idBarraPadrao[j]],vetsol[n]);
