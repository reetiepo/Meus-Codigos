...
int main(int argc, char **argv)
{
	...
	for(numArquivoEntrada = 1; numArquivoEntrada <= qtdArquivosEntrada; numArquivoEntrada++)
	{
		...
		teste = fopen("Arquivo/teste.txt", "a+");
	...
	Inicio:
			...
			while(!pare)
			{
				...
				if (acabouKmelhores){
					for(i = 0; i < (totalItensDiscretizados + qtdTiposBarras); i++)
						...
						for(i = 0; i < qtdTiposBarras; i++)
							if(amount_bar[i] > 0 && (size_bar[i] > menorItemComDemanda))
							{
								...
								for (int kk = 0; kk < k_solucoes; kk++){
									...
									cr[tamanhoCR] = matriz_K[kk].valor;//(cost_bar[i]) - matriz_K[kk].valor - multiplicadorSimplex[totalItensDiscretizados+i];
									indiceCR[tamanhoCR] = i;
									//printf("CR: %f\tBarra: %d\n", cr[tamanhoCR], size_bar[indiceCR[tamanhoCR]]);
									for (int gg = 0; gg < matriz_K[kk].tamanho_k; gg++){
										matrizKCR[tamanhoCR][matriz_K[kk].indice[gg]] = matriz_K[kk].sol[gg];
										//printf("(%d)%f\t", dados[matriz_K[kk].indice[gg]].l, matrizKCR[tamanhoCR][matriz_K[kk].indice[gg]]);
									}
									//printf("\n");
									tamanhoCR++;
								}
						...
						for (int kk = 0; kk < (k_solucoes * qtdTiposBarras); kk++)
							for (int gg = kk+1; gg < (k_solucoes * qtdTiposBarras); gg++){
								cr1 = cost_bar[indiceCR[kk]] - cr[kk] - multiplicadorSimplex[totalItensDiscretizados + indiceCR[kk]];
								cr2 = cost_bar[indiceCR[gg]] - cr[gg] - multiplicadorSimplex[totalItensDiscretizados + indiceCR[gg]];
								if (cr1 > cr2){
										trocaCR = ordenaCR[kk];
						...
						for (int kk = 0; kk < tamanhoCR; kk++){
							printf("CR: %f\tBarra: %d\n", cr[ordenaCR[kk]], size_bar[ordenaCR[indiceCR[kk]]]);
							for (int gg = 0; gg < totalItensDiscretizados; gg++){
								if (matrizKCR[ordenaCR[kk]][gg] > 0)
									printf("(%d)%f\t", dados[gg].l, matrizKCR[ordenaCR[kk]][gg]);
							}
							printf("\n");
						}
						system("pause");*/
					...
					if(!acabouKmelhores){
						crRe = 1;
						if ((indiceK < tamanhoCR))
							crRe = cost_bar[indiceCR[ordenaCR[indiceK]]] - cr[ordenaCR[indiceK]] - multiplicadorSimplex[totalItensDiscretizados + indiceCR[ordenaCR[indiceK]]];
						//verifica se o custo do novo padrão é menor do que o melhor padrão encontrado até o instante
						if((crRe < 0) && (crRe - g < 0.000001))
						{
							g = crRe;
				...
				else
				{
					...
					fprintf(teste, "indice: %d\n", indiceNovoPadrao);
					for(int kk = 0; kk < totalItensDiscretizados; kk++){
						fprintf(teste, "(%d)%d\t",dados[kk].l, matriz[kk][indiceNovoPadrao]);
					}
					fprintf(teste, "\n\n");
