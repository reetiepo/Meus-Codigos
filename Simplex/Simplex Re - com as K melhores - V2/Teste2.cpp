...
#define dirArquivoSaidaRelatorio "Arquivo/Saida/S-D4d"
...
float CalculaDirecaoSimplex(float direcaoSimplex[], vector<vector<float>> matrizInversa, float alfa[], int indice, int totalItensDiscretizados, int qtdTiposBarras)
{
	int i,j;
	float piorCR = custoAlto;
	...
	for(i = 0;i < (totalItensDiscretizados + qtdTiposBarras); i++){
		...
		if (piorCR >= direcaoSimplex[i])
			piorCR = direcaoSimplex[i];
	}
...
int main(int argc, char **argv)
{
	...
	for(numArquivoEntrada = 1; numArquivoEntrada <= qtdArquivosEntrada; numArquivoEntrada++)
	{
		...
		{			
			...
			int indiceK;
			int tamanhoCR;
			vector<int> ordenaCR(k_solucoes * qtdTiposBarras);
			vector<vector<float>> matrizKCR(k_solucoes * qtdTiposBarras, vector<float>(totalItensDiscretizados));
			vector<float> cr(k_solucoes * qtdTiposBarras);
			vector<int> indiceCR(k_solucoes * qtdTiposBarras);

			for (int h = 0; h < k_solucoes; h++){
				cr[h] = 0;
				indiceCR[h] = -1;
				for (int gg = 0; gg < totalItensDiscretizados; gg++)
					matrizKCR[h][gg] = 0;
			}
	...
	Inicio:
			...
			while(!pare)
			{
				...
				{
					float menorCustoRel = HUGE_VAL;
					float calcCustoRel = 0;
					for (int tt = 0; tt < totalItensDiscretizados; tt++){
					...
					if (acabouKmelhores){
						...
						for(i = 0; i < qtdTiposBarras; i++){
							if(amount_bar[i] > 0 && (size_bar[i] > menorItemComDemanda))
							{
								...
								for (int kk = 0; kk < k_solucoes; kk++){
									ordenaCR[tamanhoCR] = tamanhoCR;
									cr[tamanhoCR] = (cost_bar[i]) - matriz_K[kk].valor - multiplicadorSimplex[totalItensDiscretizados+i];
									indiceCR[tamanhoCR] = i;
									for (int gg = 0; gg < matriz_K[kk].tamanho_k; gg++)
										matrizKCR[tamanhoCR][matriz_K[kk].indice[gg]] = matriz_K[kk].sol[gg];
									tamanhoCR++;
								}
								indiceK = 0;
							}
						...
						int trocaCR;
						for (int kk = 0; kk < (k_solucoes * qtdTiposBarras); kk++){
							for (int gg = kk+1; gg < (k_solucoes * qtdTiposBarras); gg++){
								if (cr[kk] > cr[gg]){
										trocaCR = ordenaCR[kk];
										ordenaCR[kk] = ordenaCR[gg];
										ordenaCR[gg] = trocaCR;
								}
							}      
						}
						...
						fprintf(teste, "\n********** PADRÕES GERADOS");
						for (int re = 0; re < tamanhoCR; re++){
							fprintf(teste, "\nBarra: %d\tCR: %f\t", size_bar[indiceCR[ordenaCR[re]]], cr[ordenaCR[re]]);
							fprintf(teste, "\nPadrão:\n");
							for (int re2 = 0; re2 < totalItensDiscretizados; re2++){
								if (matrizKCR[ordenaCR[re]][re2] > 0)
									fprintf(teste, "%d(%f)\t", dados[re2].l, matrizKCR[ordenaCR[re]][re2]);					
							}
						}
						fprintf(teste, "\n*************************");
					...
					if (!acabouKmelhores)
					{
						...
						if((indiceK < tamanhoCR) && (cr[ordenaCR[indiceK]] <= 0) && (cr[ordenaCR[indiceK]] - g < 0.000001))
						{
							g = cr[ordenaCR[indiceK]];
							indice = indiceCR[ordenaCR[indiceK]];
							
							for(j = 0; j < (totalItensDiscretizados + qtdTiposBarras); j++)
								alfa[j] = 0;
								
							for (j = 0; j < totalItensDiscretizados; j++){
								alfa[j] = matrizKCR[ordenaCR[indiceK]][j];
							}
				...
				if(aux <= 0)
				{
					...
					{
						for(p = 0; p < (totalItensDiscretizados + qtdTiposBarras); p++)
						{
							if((idBarraPadrao[p] < 0 && idBarraPadrao[p] != HUGE_VAL) || solucaoInicial[p] < minPositivo)
							{
								j = p + 1;
								...
								if(j < (totalItensDiscretizados + qtdTiposBarras))
									TrocaBase(p,j,idBarraPadrao,&matriz,solucaoInicial,totalItensDiscretizados,qtdTiposBarras);
							}
						}
						
						p = 0;
			
						while(idBarraPadrao[p] >= 0)
							p++;
				...
				else
				{
					...
					fprintf(teste, "\n********** ENTROU NA BASE");
					fprintf(teste, "\nBarra: %d\tCR: %f\t", size_bar[indice], g);
					fprintf(teste, "\nPadrão:\n");
					for (int re = 0; re < totalItensDiscretizados; re++){
						if (alfa[re] > 0)
							fprintf(teste, "%d(%f)\t", dados[re].l, alfa[re]);					
					}
					fprintf(teste, "\n*************************");

					g = CalculaDirecaoSimplex(direcaoSimplex,matrizInversa,alfa,indice,totalItensDiscretizados,qtdTiposBarras);
